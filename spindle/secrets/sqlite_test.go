	...
	tests := []struct {
	...
	}{
		{
			...
			operations: []func(Manager) error{
				...
				func(m Manager) error {
					_, err := m.GetSecretsLocked(context.Background(), syntax.ATURI("at://interface.test/repo"))
					return err
				},
				func(m Manager) error {
					_, err := m.GetSecretsUnlocked(context.Background(), syntax.ATURI("at://interface.test/repo"))
					return err
				},
				func(m Manager) error {
					secret := Secret[any]{
