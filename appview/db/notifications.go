...
func (d *DB) CreateNotification(ctx context.Context, notification *Notification) error {
	query := `
...
func GetNotificationsPaginated(e Execer, page pagination.Page, filters ...filter) ([]*Notification, error) {
	...
	for rows.Next() {
		...
		n.Type = NotificationType(typeStr)
...
func GetNotificationsWithEntities(e Execer, page pagination.Page, filters ...filter) ([]*NotificationWithEntity, error) {
	var conditions []string
	var args []any
	...
	var notifications []*NotificationWithEntity
	for rows.Next() {
		var n Notification
		var typeStr string
		var createdStr string
		...
		nwe := &NotificationWithEntity{Notification: &n}
		...
		if pId.Valid {
			...
			if pState.Valid {
				pull.State = PullState(pState.Int64)
			}
			nwe.Pull = &pull
		}
...
func GetNotifications(e Execer, filters ...filter) ([]*Notification, error) {
...
func (d *DB) GetNotifications(ctx context.Context, userDID string, limit, offset int) ([]*Notification, error) {
...
func (d *DB) GetNotificationsWithEntities(ctx context.Context, userDID string, limit, offset int) ([]*NotificationWithEntity, error) {
	page := pagination.Page{Limit: limit, Offset: offset}
...
func (d *DB) GetNotificationPreferences(ctx context.Context, userDID string) (*NotificationPreferences, error) {
...
func (d *DB) UpdateNotificationPreferences(ctx context.Context, prefs *NotificationPreferences) error {
	query := `
